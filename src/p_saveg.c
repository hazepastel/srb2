...
boolean P_LoadNetGame(boolean reloading)
{
	...
	// The precipitation would normally be spawned in P_LoadLevel, which is called by
	// P_NetUnArchiveMisc above. However, that would place it up before P_NetUnArchiveThinkers,
	// so the thinkers would be deleted later. Therefore, P_LoadLevel will *not* spawn
